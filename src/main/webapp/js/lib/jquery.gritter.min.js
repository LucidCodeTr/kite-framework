		...
		function(b) {
			...
			b.gritter.add = function(f) {
				try {
					...
					(typeof (console) != "undefined" && console.error) ? console
							.error(c, f)
							: alert(c)
				}
			};
			...
			var a = {
				...
				add : function(g) {
					...
					b(
							[ "before_open", "after_open", "before_close",
									"after_close" ])
							.each(
									function(p, q) {
										a["_" + q + "_" + f] = (b
					...
					i = this._str_replace([ "[[title]]", "[[text]]",
							"[[close]]", "[[image]]", "[[number]]",
							"[[class_name]]", "[[item_class]]" ], [ k, n,
							this._tpl_close, c, this._item_count, h, m ], i);
				...
				_setFadeTimer : function(f, d) {
					var c = (this._custom_timer) ? this._custom_timer
							: this.time;
				...
				stop : function(e) {
					...
					var f = (b.isFunction(e.after_close)) ? e.after_close
							: function() {
							};
				...
				_str_replace : function(v, e, o, n) {
					var k = 0, h = 0, t = "", m = "", g = 0, q = 0, l = []
							.concat(v), c = [].concat(e), u = o, d = c instanceof Array, p = u instanceof Array;
					u = [].concat(u);
					if (n) {
						this.window[n] = 0
					}
